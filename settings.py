...
class SettingsItem:
    ...
    def draw(self, screen, font, selected):
        bg_color = ORANGE if selected else PURPLE if self.type != 'slider' else BLUE
        ...
        text = f"{self.label}: {value}" if self.type != "toggle" else f"{self.label}: {'Так' if value else 'Ні'}"
...
def settings_loop(screen, screen_width, screen_height, settings: Settings):
    ...
    items = [
        SettingsItem(
            "Гучність", "slider",
            Rect(center_x, start_y + 0 * (button_height + gap),
        ...
        SettingsItem(
            "Музика", "toggle",
            Rect(center_x, start_y + 1 * (button_height + gap),
                 button_width, button_height),
        ...
        SettingsItem(
            "Host", "text",
            Rect(center_x, start_y + 2 * (button_height + gap),
                 button_width, button_height),
        ...
        SettingsItem(
            "Port", "text",
            Rect(center_x, start_y + 3 * (button_height + gap),
                 button_width, button_height),
        ...
        SettingsItem(
            "Назад", "action",
            Rect(center_x, start_y + 4 * (button_height + gap),
                 button_width, button_height),
            get_value=lambda: "",
            set_value=None,
        ),
